...
- §3292 (bugs): remaining sibling-sweep findings from the CREDIT_LIMIT fix — both HIGH findings FIXED same day (`f6509944c` DeepSeek unknown categories, `cbe54fb3b` Perplexity grant pooling); unfixed **MEDIUM** ZaiUsageStats.swift:278 (silent drop, no log), Codebuff/Mistral throw-on-no-data, 5 LOW. Fix individually, each verified against its own fixture. Includes the parked UX decision on unlabeled icon bars.
